...
// ANSWER: no O(1) is the lowest time complexity 
...
const getNumbersSquaredBetter = (numbers) => {
  let answers = []
  for (let i=0; i>numbers.length; i++){
...
// ANSWER: the str in not being duplicated repeatedly but it also doesnt have a set limit, the size is decided by the string that is input so it must be O(n)
